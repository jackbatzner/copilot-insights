...
import { listSessions, getSessionTurns, getSessionRefs, getDb, batchGetSessionTurns } from "../src/db.mjs";
...
// ── Write mutex for file-based state ────────────────────────
// Serializes async writes to prevent read-modify-write races
...
function saveHiddenSessions(set) {
  try {
    atomicWriteFileSync(HIDDEN_FILE, JSON.stringify([...set]));
  } catch (err) {
...
function saveSessionTags(tags) {
  try {
    atomicWriteFileSync(TAGS_FILE, JSON.stringify(tags, null, 2));
  } catch (err) {
...
function writeDevPlan(data) {
  atomicWriteFileSync(DEVPLAN_FILE, JSON.stringify(data, null, 2));
...
function regenerateJournal(plan) {
  ...
  try {
    atomicWriteFileSync(JOURNAL_FILE, lines.join("\n"));
  } catch (err) {
...
app.post("/api/devplan/goals", async (req, res) => {
  try {
    await withWriteLock("devplan", () => {
      const { pillar, title, description, source, baselineScore } = req.body;
      if (!pillar || !title) { res.status(400).json({ error: "pillar and title required" }); return; }
      const plan = readDevPlan();
      const exists = plan.goals.some((goal) => goal.title === title && goal.pillar === pillar && goal.status === "active");
      if (exists) { res.status(409).json({ error: "Goal already exists" }); return; }
      const goal = {
...
app.patch("/api/devplan/goals/:id", async (req, res) => {
  try {
    await withWriteLock("devplan", () => {
      ...
      if (status) {
        if (!["active", "sufficient", "removed"].includes(status)) {
          res.status(400).json({ error: "status must be active, sufficient, or removed" });
          return;
        }
        goal.status = status;
...
app.delete("/api/devplan/goals/:id", async (req, res) => {
  try {
    await withWriteLock("devplan", () => {
      const plan = readDevPlan();
      const index = plan.goals.findIndex((goal) => goal.id === req.params.id);
      if (index === -1) { res.status(404).json({ error: "Goal not found" }); return; }
...
app.get("/api/efficiency", (req, res) => {
  try {
    ...
    if (since === null) return;
    const sessions = listSessions({ repo, since, excludeIds: hiddenSessions });
    const eligible = sessions.filter((s) => s.turn_count >= 2);
    const turnsMap = batchGetSessionTurns(eligible.map((s) => s.id));
    const sessionsData = eligible.map((s) => ({
      session: s,
      turns: turnsMap.get(s.id) || [],
