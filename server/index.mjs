...
import { listVscodeSessions, getVscodeSessionTurns, SOURCE_VSCODE } from "../src/vscode-sessions.mjs";
import { getTokenData } from "../src/token-reader.mjs";
 ...
 * Aggregate token efficiency analysis across sessions.
 */
app.get("/api/token-efficiency", (req, res) => {
  try {
    const repo = validateRepo(req, res);
    if (repo === null) return;
    const since = validateTimeframe(req, res);
    ...
    const sessionsData = sessions
      ...
      .map((s) => {
        ...
        const tokenData = getTokenData(s.id, turns);
        return { session: s, tokenData, turns };
      });
    const result = analyzeTokenEfficiencyBatch(sessionsData);
    result.pillarScore = computeTokenEfficiencyScore(result);
    res.json(result);
  } catch (err) {
    handleRouteError(res, err, "GET /api/token-efficiency");
  }
});

/**
 * GET /api/sessions/:id/tokens
 * Token efficiency for a single session.
 */
app.get("/api/sessions/:id/tokens", (req, res) => {
  try {
    const turns = getSessionTurns(req.params.id);
    if (!turns || turns.length === 0) {
      return res.json({ tokens: null, message: "No turns found" });
    }
    const tokenData = getTokenData(req.params.id, turns);
    const result = analyzeTokenEfficiency(req.params.id, tokenData, turns);
    res.json(result || { tokens: null, message: "Insufficient data" });
  } catch (err) {
    handleRouteError(res, err, "GET /api/sessions/:id/tokens");
  }
});

// —— VSCode Sessions (source filtering) ———————————————————

/**
 * GET /api/sources
 * Returns available data sources and their session counts.
 */
app.get("/api/sources", (_req, res) => {
  try {
    ...
    res.json({
      sources: [
        { id: "cli", label: "Copilot CLI", emoji: "🖥️", available: true },
        { id: "vscode", label: "VSCode Chat", emoji: "💬", available: vscodeSessions.length > 0 },
      ],
    });
  } catch (err) {
    handleRouteError(res, err, "GET /api/sources");
  }
});
...
app.get("/api/vscode-sessions", (req, res) => {
  try {
    const since = validateTimeframe(req, res);
    if (since === null) return;
    const sessions = listVscodeSessions({ since });

    const analyzed = sessions.map((s) => {
      const turns = getVscodeSessionTurns(s.id);
      ...
      for (const turn of turns) {
        ...
        if (matches.length > 0) {
          ...
          for (const m of matches) {
            ...
            if (!categoryBreakdown[m.category]) categoryBreakdown[m.category] = { count: 0, weight: 0 };
      ...
      return {
        ...
        redirectionCount,
        redirectionRate: turns.length > 0 ? redirectionCount / turns.length : 0,
        totalWeight,
        categoryBreakdown,
      };
    });

    res.json({ sessions: analyzed });
  } catch (err) {
    handleRouteError(res, err, "GET /api/sessions/vscode");
  }
});
