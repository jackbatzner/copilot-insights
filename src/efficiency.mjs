...
export function analyzeEfficiency(turns, sessionRefs = []) {
  // Only count turns with actual user-typed content (not system injections or auto-generated)
  const userTurns = turns.filter(
    (t) => cleanUserMessage(t.user_message).length > 0 && !isAutoGenerated(t.user_message)
  );
