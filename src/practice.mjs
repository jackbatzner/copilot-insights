...
/**
 * Estimate token count for a text string using a lightweight heuristic.
 * Targets ~90-95% accuracy vs. tiktoken for English + code.
 */
function estimateTokenCount(text) {
  if (!text) return 0;
  const words = text.match(/[a-zA-Z_]\w*|[0-9]+(?:\.[0-9]+)?|[^\s\w]/g) || [];
  let tokens = 0;
  for (const w of words) {
    if (/^[a-zA-Z_]\w*$/.test(w)) {
      const parts = w.split(/(?=[A-Z])|_/).filter(Boolean);
      tokens += parts.length > 1 ? parts.length * 0.8 : 1.3;
    } else if (/^[0-9]/.test(w)) {
      tokens += 1;
    } else {
      tokens += 1;
...
/**
 * Compute token efficiency estimate for a prompt in the Practice Lab.
 * Predicts the likely total token cost (including retries) and shows
 * how an optimized version would compare.
 *
 * @param {string} text - The prompt text.
 * @param {Array} matches - Redirection pattern matches.
 * @param {object} heuristics - Quality heuristic results.
 * @returns {object} Token efficiency estimate.
 */
function computeTokenEfficiencyEstimate(text, matches, heuristics) {
  const estimatedInputTokens = estimateTokenCount(text);

  // Estimate retry probability based on detected issues
  // Each pattern match increases the chance of needing a correction turn
  const patternPenalty = Math.min(matches.length * 0.2, 0.8);
  // Missing quality signals also increase retry probability
  let qualityPenalty = 0;
  if (!heuristics.hasFilePaths && heuristics.wordCount > 10) qualityPenalty += 0.1;
  ...
  // Estimate total token cost: input + expected agent response (~3x input on average)
  // plus retries: each retry costs roughly the same as the initial exchange
  const avgResponseMultiplier = 3;
  const singleExchangeTokens = estimatedInputTokens * (1 + avgResponseMultiplier);
  const estimatedTotalTokens = Math.round(
    singleExchangeTokens * (1 + estimatedRetryProbability * 1.5)
  );

  // Optimized estimate: a well-crafted prompt has ~10% retry chance
  const optimizedRetryProb = 0.1;
  const optimizedEstimate = Math.round(
    singleExchangeTokens * (1 + optimizedRetryProb * 1.5)
  );

  const savingsPercent = estimatedTotalTokens > 0
    ? Math.round(((estimatedTotalTokens - optimizedEstimate) / estimatedTotalTokens) * 100)
    : 0;

  // Grade token efficiency
  let grade;
  if (estimatedRetryProbability <= 0.15) grade = { label: "Efficient", color: "green" };
  else if (estimatedRetryProbability <= 0.35) grade = { label: "Good", color: "yellow" };
  else if (estimatedRetryProbability <= 0.55) grade = { label: "Fair", color: "orange" };
  else grade = { label: "Costly", color: "red" };

  return {
    estimatedInputTokens,
    estimatedRetryProbability: Math.round(estimatedRetryProbability * 100) / 100,
...
export function analyzePrompt(text) {
  ...
  // Flag auto-generated prompts — scores are not meaningful for these
  const autoGenerated = isAutoGenerated(text);
  ...
  // Token efficiency estimation for this prompt
  const tokenEfficiency = computeTokenEfficiencyEstimate(text, matches, heuristics);
