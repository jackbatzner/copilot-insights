...
function analyzeSessionTokensWithTurns(sessionId, turns, { includePerTurn = true, preferJsonl = true } = {}) {
  ...
  const requestCount = hasEvents
    ? realUsage.length
    : hasJsonl
      ? Math.max(jsonlData.turns.length, turnCount)
      : turnCount;
  ...
  const result = {
    ...
    avgTokensPerRequest: requestCount > 0 ? Math.round(total / requestCount) : 0,
  };
...
export function createEmptyTokenBatchAnalysis() {
  return {
    ...
    localUsage: {
      ...
      cacheHitRate: 0,
      estimatedCredits: 0,
      sourceBreakdown: { jsonl: 0, events: 0, estimated: 0 },
    },
    byModel: [],
    sessions: [],
...
export function createTokenBatchAccumulator(totalSessions = 0) {
  return {
    ...
    anyEstimated: false,
    byModel: {},
    bySource: { jsonl: 0, events: 0, estimated: 0 },
    sessionResults: [],
  };
}
...
function addTokenBatchResult(accumulator, session, result) {
  ...
  accumulator.totalPrompts += result.promptCount || result.turnCount || 0;
  accumulator.totalRequests += result.requestCount || result.turnCount || 0;
  if (result.source && result.source in accumulator.bySource) {
    accumulator.bySource[result.source] += 1;
  }

  const pricing = getModelPricing(result.model);
  const freshIn = Math.max(result.tokens.input - result.tokens.cached, 0);
  ...
  accumulator.sessionResults.push({
    ...
    avgTokensPerRequest: result.avgTokensPerRequest,
...
export function finalizeTokenBatchAccumulator(accumulator) {
  ...
  const cacheHitRate = accumulator.totalInput > 0
    ? Math.round((accumulator.totalCached / accumulator.totalInput) * 100)
    : 0;
  return {
    ...
    estimatedCredits: accumulator.totalCost / 0.01,
    costBreakdown: {
    ...
    localUsage: {
      source: "local-session-db",
      sessions: analyzed,
      prompts: accumulator.totalPrompts,
      requests: accumulator.totalRequests,
      avgTokensPerPrompt: accumulator.totalPrompts > 0 ? Math.round(total / accumulator.totalPrompts) : 0,
      avgTokensPerRequest: accumulator.totalRequests > 0 ? Math.round(total / accumulator.totalRequests) : 0,
      cacheHitRate,
      estimatedCredits: accumulator.totalCost / 0.01,
      sourceBreakdown: { ...accumulator.bySource },
    },
    avgTokensPerSession: analyzed > 0 ? Math.round(total / analyzed) : 0,
    avgCostPerSession: analyzed > 0 ? accumulator.totalCost / analyzed : 0,
    byModel: Object.values(accumulator.byModel)
      .map((model) => ({
        ...model,
        avgTokensPerSession: model.sessions > 0 ? Math.round(model.total / model.sessions) : 0,
        cacheHitRate: model.input > 0 ? Math.round((model.cached / model.input) * 100) : 0,
      }))
