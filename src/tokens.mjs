...
import { SESSION_STATE_PATH, listSessionJsonlFiles, parseJsonlFile } from "./session-state.mjs";
...
function getCachedSessionTokenAnalysis(sessionId) {
...
function setCachedSessionTokenAnalysis(sessionId, value) {
  sessionTokenCache.set(sessionId, { value, ts: Date.now() });
}
...
function readSessionTokensFromJsonl(sessionId) {
  ...
  for (const file of jsonlFiles) {
    const objects = parseJsonlFile(file);
...
export function analyzeSessionTokens(sessionId) {
  const cached = getCachedSessionTokenAnalysis(sessionId);
  ...
  setCachedSessionTokenAnalysis(sessionId, result);
  return result;
}
